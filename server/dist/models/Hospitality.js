...
const HospitalitySchema = new mongoose_1.Schema({
    companyId: {
...
HospitalitySchema.pre('save', function (next) {
    ...
    this.activeBookings = this.bookings.filter(booking => ['confirmed', 'checked_in'].includes(booking.status)).length;
...
HospitalitySchema.methods.getAvailableRooms = function (checkIn, checkOut, roomType) {
    return this.rooms.filter(room => {
        ...
        const isBooked = this.bookings.some(booking => {
            if (booking.status === 'cancelled' || booking.status === 'no_show')
                return false;
            if (!booking.roomDetails.roomNumbers.includes(room.roomNumber))
                return false;
            return ((checkIn >= booking.stayDetails.checkInDate && checkIn < booking.stayDetails.checkOutDate) ||
...
HospitalitySchema.methods.calculateRevenue = function (startDate, endDate) {
    const relevantBookings = this.bookings.filter(booking => booking.stayDetails.checkInDate >= startDate &&
...
HospitalitySchema.methods.getOccupancyRate = function (date) {
    const totalRooms = this.rooms.length;
    if (totalRooms === 0)
        return 0;
    const occupiedRooms = this.bookings.filter(booking => booking.status === 'checked_in' &&
        booking.stayDetails.checkInDate <= date &&
        booking.stayDetails.checkOutDate > date).reduce((sum, booking) => sum + booking.roomDetails.totalRooms, 0);
    return (occupiedRooms / totalRooms) * 100;
};
HospitalitySchema.statics.findByCompany = function (companyId) {
    return this.find({ companyId, isActive: true });
};
HospitalitySchema.statics.getHospitalityStats = function (companyId) {
...
exports.default = (0, mongoose_1.model)('Hospitality', HospitalitySchema);
