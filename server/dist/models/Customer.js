...
const CustomerSchema = new mongoose_1.Schema({
    ...
    relationship: {
        ...
        assignedSalesPerson: { type: mongoose_1.Schema.Types.ObjectId, ref: 'User' },
...
CustomerSchema.methods.getPrimaryAddress = function (type) {
    if (type) {
...
CustomerSchema.methods.canPlaceOrder = function (orderValue) {
    if (this.compliance.blacklisted)
        return false;
    if (!this.isActive)
        return false;
...
CustomerSchema.statics.findByCompany = function (companyId) {
    return this.find({ companyId, isActive: true });
};
CustomerSchema.statics.findByType = function (companyId, customerType) {
    return this.find({
        companyId,
...
CustomerSchema.statics.findOverdue = function (companyId) {
...
CustomerSchema.virtual('isOverdue').get(function () {
    if (!this.financialInfo?.lastPaymentDate || !this.financialInfo?.creditDays)
        return false;
    const dueDate = new Date(this.financialInfo.lastPaymentDate);
    dueDate.setDate(dueDate.getDate() + this.financialInfo.creditDays);
...
CustomerSchema.query.byCompany = function (companyId) {
    return this.where({ companyId });
};
CustomerSchema.query.active = function () {
    return this.where({ isActive: true });
};
CustomerSchema.query.withOutstanding = function () {
    return this.where({ 'financialInfo.outstandingAmount': { $gt: 0 } });
};
CustomerSchema.query.overCreditLimit = function () {
    return this.where({
        $expr: {
