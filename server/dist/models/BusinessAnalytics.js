...
const KPIMetricSchema = new mongoose_1.Schema({
...
const BusinessAnalyticsSchema = new mongoose_1.Schema({
    companyId: {
    ...
    dataRetention: {
        ...
        reportDataDays: { type: Number, default: 1095 },
...
BusinessAnalyticsSchema.methods.getMetricByCode = function (metricCode) {
    return this.kpiMetrics.find((metric) => metric.metricCode === metricCode);
};
BusinessAnalyticsSchema.methods.updateMetricValue = function (metricCode, newValue) {
    ...
    if (metric) {
        ...
        if (metric.changePercentage > 5)
            metric.trend = 'up';
        else if (metric.changePercentage < -5)
            metric.trend = 'down';
        else
            metric.trend = 'stable';
...
BusinessAnalyticsSchema.methods.getDashboardByType = function (dashboardType) {
    return this.dashboards.find(dashboard => dashboard.dashboardType === dashboardType && dashboard.isActive);
};
BusinessAnalyticsSchema.statics.findByCompany = function (companyId) {
    return this.find({ companyId, isActive: true });
};
BusinessAnalyticsSchema.statics.getKPISummary = function (companyId) {
...
exports.default = (0, mongoose_1.model)('BusinessAnalytics', BusinessAnalyticsSchema);
